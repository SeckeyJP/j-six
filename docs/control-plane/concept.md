...
Hub が判断する適合性は、対象の成果物と適用するルールの版を指定し、次の3つに分ける。
これは構想の保証範囲を明確にするための著者提案であり、実装・実証済みの保証ではない。

| 区分 | 判断すること | 必要な記録 | その記録だけでは分からないこと |
|---|---|---|---|
| 成果物の適合 | 対象の成果物が、指定した検査条件を満たしたか | 対象のコミット・成果物、検査と設定の版、実行環境、結果 | 承認前に実装を始めなかったか、Red → Green の順に作業したか |
| 作業履歴の適合 | 記録された作業が、必要な前提と順序に従ったか | タスク・実行の識別子、開始・終了・判定の記録、先行する承認・検査への参照 | 記録されていないローカル作業の順序、記録の外で行われた操作 |
| 承認手続の適合 | 対象と版を特定した承認を、権限のある人間が行い、現在も有効か | 承認対象、承認者の確認可能な識別情報、権限の根拠、承認・失効の記録（§2.2） | 成果物の欠陥が無いこと、実案件での効果 |

各区分について、確認できた範囲・違反・未検証の範囲を別々に示す。記録が無いことを違反なしとは扱わない。
例外を認めた場合も元の不合格・未検証を残し、誰が何をどの条件で受け入れたかを追加する。
工程を先へ進められるかは、適用するルールの必須条件と、有効な例外承認を合わせて判断する。
必要な記録が不足し、有効な例外承認も無い場合には、自動で適合済みとして進めない。

中央実行であっても、記録の完全性と実行・承認主体の識別を確認できる範囲に限って判断する。
リプレイ型サンプルは過去の記録の可視化と構想の説明を行うもので、この保証条件を実行時に強制する基盤ではない。

### 2.2 承認・検査結果の対象と有効性（著者提案）

Git を正本にすることは保存と再構築の方針である。記録の内容が真正で、現在の成果物に有効であることは別に確認する。
以下を将来の Hub に必要な記録と判定規則とする。保存用のスキーマや認証方式の選定は別途行う。

| 項目 | 記録する内容 |
|---|---|
| 対象 | Project・Phase・ゲート、必要に応じ Task・実行 ID、リポジトリ、対象コミットと成果物のパス・内容ハッシュ |
| 適用ルール | プロセス定義、憲法、検査スクリプト・設定、関係する Interface Contract の版 |
| 主体 | 承認者または検査実行主体の識別情報、当時の役割・権限とその根拠、記録の発行元 |
| 判断 | 承認・却下・検査結果、時刻、判断理由、根拠となる記録への参照 |
| 有効性 | 失効・取消し・後継記録への参照。例外では対象条件、承認者、期限・解消条件 |

人間の承認は既存の認証・権限基盤で確認できる操作を根拠とし、その根拠を Git に保存する。
AI がファイルに記入した名前、`human` ラベル、コミットの著者名だけでは、人間が承認した証明にしない。
検査も、対象変更が任意に書き換えられる結果ファイルだけを信頼せず、許可された発行元と対象の対応を確認する。
外部サービスの可変な参照先だけに依存せず、検証に必要な記録と当時の権限の根拠を保存し、
Hub の読み取りモデルを失っても対象別の承認・検査・例外の状態を再構築できるようにする。
信頼境界と主体別の権限は §2.3 と ADR-0006 に定める案である。本人確認・署名・保管方式と既存基盤への具体的な設定は、試作前に確認する。

- 対象の成果物、その実行に適用されるルール、検査対象の依存関係が変わった場合、旧承認・旧検査を新しい対象に流用しない。
  変更の影響範囲を記録し、該当する再検査・再承認が完了するまで未検証とする。影響を判断できない範囲も未検証にする。
- 過去の承認・結果は削除せず、当時の対象には有効だった事実と、現在の対象には適用できない状態を区別する。
  承認記録を追加しただけのコミットなど、対象成果物とルールが変わらない変更は、それだけで再承認を要求しない。
- タスク投入・工程遷移・マージの直前に、対象と必要な承認・検査・例外の有効性を再確認する。
  確認後に対象が変わった場合には、その許可を取り直す。並行操作で古い判断を適用しないことを設計上の条件とする。
- 承認・例外は Git の正本に記録されてから、次の作業を許可する根拠に使う。書込みが失敗した場合は承認済みにしない。
- 例外の期限切れ・取消し・対象の変更後は、再判定または新たな例外承認を要する。例外は検査結果を合格に書き換えない。

例：要求 Spec の版 A を承認した後に版 B へ改訂した場合、A の承認は履歴として残し、B の承認には使わない。
B の影響を受ける設計・タスク・検査を特定して再確認する。ゲート通過後にコードが変わった場合も、
旧コードに対する合格を新コードの合格として扱わない。

### 2.3 信頼境界と必須検査（著者提案・未実装）

作業 AI は候補成果物と変更提案だけを書き、検査方針・承認・検査結果の有効な発行者を兼ねない。
独立した検査主体は保護された有効方針を読み、候補ブランチの設定をそのまま実行基準にしない。
Hub の判断処理は既存基盤で確認できる主体・対象・版・記録を照合し、記録サービスが Git に保存した
許可だけを実行側へ渡す。ポリシー管理者・認証基盤・Git 保護設定は信頼の前提であり、Hub 自身が保証するものではない。
主体別の読み書き権限と迂回経路の確認は [ADR-0006](adr/0006-trust-boundaries-and-policy.md) に定める。

有効方針には、Phase・タスク種別に応じた必須検査、任意検査、適用条件、例外を許せない条件を明記する。
判断時は方針側の必須集合から結果を照合し、結果に載っている検査だけを集計しない。

| 状態 | 判定 |
|---|---|
| passed | 対象・版・発行元・環境が一致し、必須集合を満たした範囲の根拠 |
| missing | 必須設定または結果が欠落。未検証であり許可しない |
| skipped | 未実行。成功の終了コードでも必須条件を満たさない |
| excluded / not-run | 対象外指定または前段失敗による未実行。必須集合から取り除かない |
| failed / error / unknown | 未達・実行異常・未知の検査。許可しない |
| not applicable | 有効方針の適用条件に基づく非該当理由を記録。実行者が任意に設定しない |

例外を許す条件は、対象・期限・承認権限を検証した別記録を要する。欠落した検査を passed へ変更しない。
Plugin は未知チェックを拒否するが、未設定プロジェクトの no-op や任意検査のスキップは互換性のため残る。
その終了コードだけを Hub の進行許可に使わない。この独立照合は将来の Hub の実装義務である。

---

試作前の対象・方針・来歴・権限・失効・競合の判定条件を[実行・判定契約](execution-contract.md)へ具体化した。[合成シナリオ](decision-scenarios.md)と[ADR-0008](adr/0008-evidence-envelope.md)は未実装の設計案で、既存のリプレイの保証ではない。
...
**通常の人間承認は Phase 境界に限定する**。P4 のタスク単位のゲート（G1〜G4）には定常的な人間承認を置かない
（J-SIX.md の設計どおり）。例外承認・緊急失効・契約変更の判断は、該当時だけ発生する別手続である。承認を増やすと統制が過剰になり、承認待ちが工程を止めるためである（§8）。
...
| LLM 判定 | G3 scope-judge | 再実行で結果が変わりうる | 参考所見（肯定だけで最終的な品質合格の根拠にしない。否定は保守的な一時停止の条件とする） |
...
### 4.2.1 実行要求と復旧（著者提案・未実装）

工程の状態を再構築できても、外部で起動した実行の有無まで確定できるとは限らない。
実行要求を Git に保存してから外部へ送り、安定した run ID と操作ごとの command ID で起動・結果・PR を照合する。
結果不明時は `unknown → reconciling` として停止し、外部状態と照合するまで同じ仕事を新規投入しない。
取消しは要求と完了確認を分け、確認できない結果を取り込まない。
実行記録の順序、再送、部分完了、失効との関係は [ADR-0007](adr/0007-execution-recovery.md) に定める。
...
| ローカル退避 | 中央実行では解決できず、人間がローカル CC で作業する | 成果物を PR としてゲートに再投入し、未検証の履歴は区別して回収する（§4.3.1） | 退避の開始・終了、対象とルールの版、再判定の結果、未検証の範囲と例外対応 |
...
例外として通した」という記録を承認区分に追加する。保存した実行結果とその決定論的な集計を維持するためである。検査を再実行すると環境・乱数・外部状態等で結果が変わり得るため、新しい実行結果として記録する（J-SIX.md 9.1）。

### 4.3.1 ローカル退避からの回収

ローカルから戻す際は、対象コミットと適用ルールを固定し、定められた成果物の検査を必ず再実行する。
この再検査が示すのは成果物の適合であり、ローカルでの作業順序の適合を遡って証明するものではない（§2.1）。

退避の開始・終了、理由、担当者、対象タスク、持ち帰った成果物とルールの版、再検査結果を記録する。
作業履歴は、確認できるものを根拠とともに取り込み、確認できない区間を未検証として残す。
不足する履歴が工程の必須条件である場合は、可能な作業をやり直して記録を得るか、例外対応として
権限のある人間が未検証の範囲と残るリスクを明示して受け入れる。受入れが無ければ先へ進めない。
これは通常のタスクゲートに一律の人間承認を追加するものではなく、既存の例外対応を具体化するものである。
例外を認めても、失われた作業履歴を適合済みとして補完しない。
...
複数ベンダーにまたがる変更は、将来の Hub では次のように扱う案である。これは実装済みのワークフローではない。

1. 所有者が変更案を契約の候補版として登録し、破壊的変更か、影響を受ける利用者・成果物・移行期限を記録する。
2. 利用者は候補版ごとに影響分析を行い、互換アダプタ、段階移行、同時リリース、または変更拒否を選ぶ。利用側の
   タスク・Spec・テスト・受入条件を変更案に紐付け、未回答の利用者を「影響未確認」として残す。
3. 所有者と利用者が合意した移行計画に従い、旧版・新版の互換性検査と、利用側の結合検査を再実行する。
   契約の差分だけで利用側が受け入れ可能とは判断しない。
4. 全利用者の移行結果、未移行の例外、ロールバック方法を確認し、権限を持つ利用側の受入れと所有者の変更承認を
   記録してから、新版を既定にする。旧版を廃止する時点でも、残存利用者・期限・例外を再確認する。

この間、変更前の契約版と承認・検査結果は履歴として残す。影響が未確認、利用側の再検査が未完了、または
受入れの根拠が無い変更を、所有者だけの承認で他ベンダーへ強制適用しない。実際の責任分界・契約条項・
リリース権限は案件ごとに異なるため、Hub がそれらを置き換えるとは主張しない。

#### 運営・開示・利用者登録の前提（著者提案）

最初の試作対象は、共通の運営主体を合意できる Program に限る。元請・顧客のどちらが担うかは案件で明記し、
各参加組織が認めた運営責任者、ポリシー管理者、契約所有者、利用側受入れ責任者を登録する。
Hub の運営主体であることだけでは、他組織の成果物の承認権限を得ない。

| 対象 | 保管・共有と責任 |
|---|---|
| ソース・完全な作業履歴 | 各組織の Git を正本とする。全員への公開を前提にしない |
| Program で共有する証拠 | 契約版、対象ハッシュ、適用方針、必須検査の一覧と結果、発行元、承認・例外・移行状態を最小限のマニフェストとして共有 |
| 閲覧者 | 契約所有者・関係利用者・運営担当など、案件で認めた主体に限る。機密の元データは権限を持つ検査主体が検証する |
| 検証できない証拠 | ハッシュの一致だけでは内容の正しさを証明しない。元データへのアクセスまたは信頼できる検査発行者を確認できなければ未検証とする |
| 利用者台帳 | 所有者が管理し、各利用側責任者が直接・間接依存と下請けへの影響を申告・確認する。版・対象範囲・確認者・確認日を保存 |

「全利用者」は台帳の版と確認済み範囲を伴う条件とする。登録範囲が不明なら影響確認未完了とし、Program 全体の
新版への切替え・旧版廃止を止める。未回答は承認に読み替えない。案件で応答期限と上位の調整責任者を決め、
期限超過時は調整へエスカレーションする。旧版併存・限定利用者への段階移行は、確認できた範囲と
他の利用者への影響を遮断する条件を記録して行う。上位判断でも未検証を合格に書き換えず、権限のない代諾をしない。
これらは試作の前提であり、実組織で合意された責任分界ではない。

### 5.3 憲法（CLAUDE.md）の版管理と伝播

- CLAUDE.md は Git で版管理し、Hub は各タスクに**どの版の憲法を渡したか**を記録する
- 通常の憲法改訂（Phase 0 の月次ループ）は、以後に投入するタスクから適用する。既存 run の固定版は変更しない。§2.2 の再検査は、その run の対象・有効方針が変わる場合に適用する
...
緊急失効は通常改訂と分ける。登録した失効権限者が旧方針・許可の対象、発効時点、理由、対象 run を Git に記録する。
Hub は新規投入と結果取込みを止め、実行側へ取消しを要求する。停止確認までを未確定として表示し、実行中・遅着の結果を隔離する。
旧版の記録を変更せず、新方針で再検査・必要な再承認を得て、新しい許可と run ID で再開する（ADR-0006・0007）。
組織 → Program → Project → Task の下位方針は上位の禁止事項を緩められない。上位で例外可能と明記した条件だけ、
権限・対象・期限付きの別記録で例外を扱う。例外不可の条件や権限境界をタスクの指示で上書きしない。
...
| H-4 | 通常の人間承認を Phase 境界に限定すれば、統制による人間の負荷（承認回数・待ち時間）は許容範囲に収まる |

### 7.2 リサーチクエスチョン

| RQ | 問い | 指標（案） |
|---|---|---|
| RQ1 | Hub による統制は、プロセス適合性を高めるか | Phase 順序違反の件数、ゲート迂回（ゲートを通らずにマージされた変更）の件数、誤停止・見逃しの件数 |
| RQ2 | 証跡とトレーサビリティは完全になるか | REQ・PROP ⇔ テスト ⇔ コードの対応の欠落数、証跡パッケージの欠落数 |
| RQ3 | 逸脱は回収されるか | 逸脱の種類別の件数、回収完了率、回収までの時間、回収されず残った逸脱の件数 |
| RQ4 | 統制のコストはどれだけか | 人間承認の回数と待ち時間、エスカレーション件数、トークン使用量。人間が関わる比較実験でのみ H-4 を評価する |
...
- **比較条件**：統制あり（Hub がタスク投入、工程遷移、逸脱回収を管理）／Plugin 条件（各エージェントが
  J-SIX Plugin を個別に使う）／リポジトリ保護条件（Plugin に加え、必須 CI と PR 承認だけを適用する）。
  最後の条件はマージ時の関門と、工程全体を追跡する統制との差を調べる基準である。各条件で同じモデル、
  タスク、時間・トークン予算、許可範囲、検査内容と有効方針を固定する。最初の比較では同じ実行イメージ・依存関係・ツール版・ネットワーク許可・計算資源・乱数条件・回復用資源と予算も揃える。変えるのは投入・遷移・マージ・回収に対する強制の経路とタイミングとする。環境のばらつきは別の実験要因に分ける
- **効果比較前の適合性検査**：合成入力で未知・欠落・空設定・スキップ・対象外検査、並行例外・二重終了、古い承認、権限外の方針変更、緊急失効、起動・PR作成・承認保存前後の障害を検査する。期待する許可／保留と復旧結果を事前固定する。実データの効果比較はその後に行う
- **題材**：J-SIX の既存サンプル（`examples/approval-workflow`、`examples/monthly-billing`）を、
  複数チームに分割した構成
- **測定**：RQ1〜RQ3 の指標。停止は正しく止めるべき変更を止めなかった「見逃し」と、正しく進められる変更を
  止めた「誤停止」に分ける。各タスクの完了率、逸脱の回収完了率と回収時間も記録する。逸脱は、模擬開発者の
  設定から意図的に起こすものと、自然に起きたものを分けて数える
- **集計単位**：独立した違反 ID と繰返しブロック回数を別に数える。見逃し率の分母は事前に違反と判定した機会、誤停止率の分母は許可すべき機会とし、同じ停止の再送を新しい機会にしない。完了率の分母は投入した全 run。回収完了率は回収対象の全逸脱を分母とし、打切り時点の未回収・状態不明・取消しも内訳を残す。回収時間は完了例と未完了例の観測期間を分け、成功例だけの平均で比較しない
- **限界**：模擬開発者は人間の開発者の行動を再現しない。この実験が示せるのは「統制の仕組みが意図どおりに
  作動するか」であり、実際の組織で効果があるかではない。人間の承認回数・待ち時間・受容可能性を測れないため、
  H-4 と RQ4 の人間負荷はこの予備実験の対象外である

---

指標の分母・全投入の費用・打切り・料金方式は[評価設計](../evaluation-design.md)に従う。RQ2ではID対応の存在と意味上の充足を区別し、RQ4では実作業と待ち時間を分ける。合成判定器の比較は実AI・実Hubの強制力の実証ではない。現段階では企業の実PJ・人間を含む比較は行わず、H-4は未評価とする。
...
| セキュリティ | 顧客のソースコードを AI の API に送信する | J-SIX.md 第7章のリスク（コード外部送信）と同じ。Execution Plane でのデータの扱い（保持・破棄）に加え、組織間の証拠開示・閲覧者・未検証時の扱いは §5.2、実行権限は §2.3 を適用する |
| コスト | タスクごとのコンテナ・トークン消費が案件全体で積み上がる | トークン予算をタスク・Project 単位で設定し、超過をエスカレーションにする案がある（未検討） |
| 統制過剰 | 承認・ゲートが増えると工程が止まり、統制を回避する運用が生まれる | 通常の人間承認を Phase 境界に限定する（§3）。承認待ち時間をメトリクスにする（RQ4） |
| PSEE 型の硬直 | 想定外の状況を扱えず、プロセスが現場から乖離する | 逸脱を状態モデルに含める（§4.3）。ただしこれで硬直を避けられるかは検証されていない |
| LLM 判定の非決定性 | G3 の結果は再実行で変わりうる | G3 は参考所見として扱い、肯定だけで最終的な品質合格の根拠にしない。否定は保守的な一時停止の条件とする（J-SIX.md 9.1） |
