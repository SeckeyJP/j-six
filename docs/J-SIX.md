...
| mutation score（十分条件側） | 通常は未計測 | 閾値は実測後に設定 | —（※7） | 🟡 未実測 |
...
> ※7 カバレッジは「テストが実行した行」を測るが、「テストが誤りを検出できるか」は測らない。後者を測るのが mutation score（意図的にコードを改変し、テストが失敗するかを見る指標）である。**実測前に閾値の既定値は置かない**方針とし、ケーススタディ #2 で「カバレッジ 99% のコードの mutation score」を実測してから基準値を決める
...
| 実証 | ケーススタディ #2（`docs/case-study-02.md`） | カバレッジ 99% のコードの mutation score を実測、PBT で何が見つかったか | ⏳ v2.1 で実施 |
...
| 8 | 4層品質ゲート（G1-G4）の Plugin 実装 | GitHub（Plugin scripts / Agents / Skills） | ⏳ v2.1 で実装中 |
| 9 | 証跡パッケージ（顧客納品対応） | GitHub（evidence-pack Skill） | ⏳ v2.1 で実装中 |
| 10 | mutation testing × PBT の実測（ケーススタディ #2） | 記事 + GitHub | ⏳ v2.1 で実施 |
