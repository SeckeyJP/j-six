...
G3は任意構成だが、省略時は最大L3である。L4はG1〜G4全層通過を必要とする。
`optional: true` はL4の免除を意味しない。yamlの説明は分類規則であり、Pluginが自律度を機械的に認定する実装ではない。
Hubの既存固定タグは変更せず、取込版の更新は別途判断する。
...
### 当時のJ-SIX.md内の不整合の候補（履歴）

項目7は2026-09-26に上記の最大L3条件を本文/yamlへ明記して解消。以下は当時の記録。
