...
### Changed
- ローカル合成検証36ケース、候補採点11例、Plugin実Git検査、Word/Excelの限定的な生成・表示確認と再実行コードを追加。未実施の実AI比較と企業PJ適用を明示。
- 工数換算と現金支出を分ける評価設計・記録ひな型・仮定の計算例を追加。Stageの推定削減率を合格条件から分離し、品質要求と代理指標の扱いをprocess説明にも反映。
