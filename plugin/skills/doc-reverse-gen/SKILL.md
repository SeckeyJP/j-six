...
機械抽出とLLMによる記述・要約・変換を区別する。由来がコードでも誤解釈や省略は起こりうる。
元コミット（未コミットなら内容ハッシュ）、参照元、生成方式/版、確認者・結果を記録する。
元データ変更後は影響する文書を未確認とし、再生成・再レビューする。要求の正しさは逆生成で証明しない。
...
転記を減らせるが、対象版の一致・記述の正確性・省略・変換誤りは別に確認する。
...
本Pluginには専用の形式変換器を同梱していない。案件で利用可能な変換ツールを確認し、版・実行手順を記録する。
使える変換器や表示確認手段が無ければMarkdownまでとし、未対応/未確認と報告する。
変換後は表・図・改ページ・由来・数値を確認し、成功したファイル形式だけを対応済みとする。
