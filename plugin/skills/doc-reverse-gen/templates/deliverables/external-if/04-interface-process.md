<!-- 自動生成: tools/sync_plugin_templates.py が templates/deliverables/external-if/04-interface-process.md から生成。直接編集せず、正のファイルを直して再生成すること -->
# 外部インタフェース処理説明
...
- **書き落としやすい点**: スキップと打ち切りの違い。`continue` を `break` にしても多くの
  テストは通る。mutation testing で検証し、結果をここに残す。もう1つは**列数が合わない行**の
  扱い。CSV ライブラリの既定の挙動（不足列を空値で埋める等）に任せると、1行の不備で取込全体が
  止まったり黙って取り込まれたりする。テストデータが常に正しい列数だとカバレッジも mutation も
  これを検出できない（実例ではコードレビューで見つかった）。
- **記入済み実例**: [月次請求書発行](https://github.com/SeckeyJP/j-six/blob/main/examples/monthly-billing/docs/deliverables/external-if/04-interface-process.md)
