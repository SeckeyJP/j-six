...
Plugin 2.1.3 では、Git 比較・特殊文字を含むパス・JUnit 入力・承認対象の鮮度を
厳密に検査します。承認は commit、未コミット差分、設定とゲート結果に結び付け、
変更後は旧記録を残して再承認を求めます。`--run-commands` 時は coverage を含む
各コマンドを一度実行してからレポートを読みます。
Green 用 Hook が直接遮断するのは対象ツールの操作に限られ、hold-out の完全な
不可視性を保証するものではありません。
...
G1・G2 の未知チェック名は設定エラーとして拒否する。runner を正規化なしで直接呼び出しても未知チェックは不合格になる。
設定のないプロジェクトの no-op、任意検査の未実行、`--gates` による対象外指定は引き続き許容する。この成功終了は必須検査を網羅した証明ではない。
将来の Hub 等で進行許可に使う場合は、作業側が改変できない方針の必須集合を起点に、対象・版・発行元を確認した結果と照合する。
