...
def test_uc004_approver_rejects_and_it_is_terminal(client):
    ...
    res = client.post(
        f"/requests/{rid}/reject", json={"actor": "bob", "note": "予算超過"}
    )
    ...
    assert (
        client.post(f"/requests/{rid}/approve", json={"actor": "bob"}).status_code
        == 409
    )
    assert (
        client.post(f"/requests/{rid}/withdraw", json={"actor": "alice"}).status_code
        == 409
    )
...
def test_uc005_remand_returns_to_draft_and_restarts(client):
    ...
    res = client.post(
        f"/requests/{rid}/remand", json={"actor": "carol", "note": "見積添付漏れ"}
    )
    assert res.is_success, res.text
