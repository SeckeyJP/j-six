...
本書は v2.0 の構成で作成しており、非機能設計の節は6大項目に揃えていない（5.1・5.2 は
...
| 運用・保守性 | 全状態遷移を監査ログに記録（REQ-010）。タイムスタンプは `clock` 注入で検証可能（5.1） |
| 移行性 | 過去の申請データは移行しない |
| セキュリティ | 自己承認の禁止（REQ-007）と承認順序の強制（REQ-008）をドメイン層で強制。認証は対象外 |
| システム環境・エコロジー | 対象外 |
