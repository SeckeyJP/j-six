...
| 2 | UC-002, UC-003 | 提出 → 順序どおりの多段承認 → APPROVED、監査ログの並び。提出・承認で付けたコメントが監査ログに残ること |
| 3 | UC-004, UC-005 | 却下の終了性、差し戻し後の再提出で最初の承認者に戻ること |
| 4 | UC-006 | DRAFT / PENDING からの取下げ、申請者以外の拒否。取下げで付けたコメントが監査ログに残ること |
| 5 | UC-002〜UC-006（横断） | 存在しない申請への状態遷移が「対象不在」になること（REQ-011）。未定義の項目を含む入力が「入力不正」になること（REQ-012） |
...
- [x] 3.1 ユースケース・3.2 業務ルール・3.3 受入条件・3.5 非機能要件が、発注者・開発者の双方で
      確認済みで、版管理されている（**完成レベル**）
- [x] 未確定の項目は「未確定」と明記され、確定させる Phase と担当者が書かれている（本版に未確定の項目はない）
...
|---|---|---|
| 1.0 | 2026-06-14 | 初版 |
| 1.1 | 2026-09-19 | 設計レビュー（`docs/reviews/design-review-2026-09-19.md`）の C-1 / C-2 を受けて改訂。REQ-010 に状態遷移のコメント記録を追加。REQ-011（対象不在の区別）・REQ-012（未定義の入力項目の拒否）と PROP-007〜PROP-009 を追加。hold-out の観点を追加 |
