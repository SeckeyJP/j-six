...
└── docs/
    ├── requirement-spec.md   # UC 6 / REQ 13 / PROP 8
...
| 指標 | 実測値 |
|---|---|
| アプリ実装 | 352 ステートメント（app/） |
| テスト件数 | 111 件（例ベース + 性質）＋ hold-out 受入 23 件 |
| ステートメントカバレッジ | 99.7% |
...
| 明細の `quantity=sale.quantity` | 売上から明細への数量の転記が未検証（帳票に出る） |
| `reset` の `_seq = 0` | reset 後に採番が 0001 から始まり直すことが未検証 |
...
> 上の 87.89% → 92.95%（454 ミュータント中 422 killed）は 2026-09-18 時点の経過。その後、
> form 解析を標準ライブラリに置き換え（ADR-0004）、列数不一致の CSV 行をエラー行として扱う修正を
> 入れた（テストは 93 件から 101 件に増えた）。コードが増えてミュータントも増え、2026-09-19 の
> 再計測は 91.88%（505 ミュータント中 464 killed／生存41件）。さらに TASK-MB-007（振込先口座）を
> 実装した 2026-09-21 の再計測が、計測結果の表に示した 91.49%（552 ミュータント中 505 killed／生存47件）である。閾値 90% は引き続き満たす。

#### TASK-MB-007（振込先口座）で見つかった穴

| 生存ミュータント | 意味 |
|---|---|
| `BankAccount(account_type=None)` | **振込先の預金種別が落ちても気づけない**。金融機関名・支店名・口座番号・名義は検証していたが、預金種別だけ検証が無かった（帳票に印字される項目）。単体テスト1件を追加して塞いだ |
...
27点中19点が実物から起こせる。**人手更新が必要なのは「なぜ作るか」「どう運用するか」を
