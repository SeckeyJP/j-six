...
| UC-004 | 請求書を出力する | 経理担当者 | 適格請求書の記載事項を満たす帳票を出力 | 高 |
...
| カテゴリ | 要件 | 目標値 | 根拠 |
|---|---|---|---|
| 性能 | 月次締めの処理時間 | 取引先1,000件・売上10万件で5分以内（本サンプルはインメモリのため計測対象外） | 経理の締め作業時間 |
| 品質 | ドメインロジックのテストカバレッジ | 95% 以上 | J-SIX 品質ゲート |
| 品質 | mutation score | 90% 以上 | ケーススタディ #2 の実測に基づく（本サンプルでも同基準を適用） |
| セキュリティ | 締め処理の証跡保全 | 全締め処理の監査ログ必須（REQ-010） | 内部統制 |
